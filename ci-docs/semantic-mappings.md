...
1. Перейдіть до **розділу Семантичне відображення даних** > **(попередній перегляд)**.
...
## <a name="use-a-contactprofile-semantic-entity-mapping-to-create-contact-level-activities"></a>Використання семантичного зіставлення сутності ContactProfile для створення заходів на рівні контактів

Після створення семантичного *зіставлення сутності ContactProfile* можна фіксувати діяльність контактів. Це дає змогу бачити в часовій шкалі активності облікового запису, контакт якого відповідав за кожну діяльність. Більшість кроків відповідають типовій конфігурації мапінгу діяльності.

   > [!NOTE]
   > Щоб дії на рівні контактів працювали, необхідно мати **атрибути AccountID** і **ContactID** для кожного запису в даних про активність.

1. [Визначте відображення семантичної *сутності ContactProfile* .](#define-a-contactprofile-semantic-entity-mapping) і запустити семантичне відображення.

1. Перейдіть до **розділу Дії з** > **даними**.

1. Виберіть елемент **Додати активність**, щоб створити нову дію.

1. Назвіть вправу, виберіть сутність вихідної діяльності та виберіть первинний ключ суб'єкта діяльності.

1. На кроці "Зв'язки **"** створіть непрямий зв'язок між даними джерела вашої активності з обліковими записами, використовуючи свої контактні дані як організацію-посередника. Для отримання додаткової інформації дивіться [шляхи прямих і непрямих](relationships.md#relationship-paths) відносин.
   - Приклад зв'язку для діяльності під назвою *"Покупки"*:
      - **Придбання контактних даних** > **про** активність джерела за атрибутом **ContactID**
      - **Дані** > **облікового запису контактних даних** за атрибутом **AccountID**

   :::image type="content" source="media/Contact_Activities1.png" alt-text="Приклад налаштування зв'язку.":::

1. Настроївши зв'язки, натисніть кнопку **Далі** та завершіть конфігурацію зіставлення дій. Детальні кроки щодо створення діяльності дивіться у [визначенні діяльності](activities.md).

1. Запустіть зіставлення активності.

1. Ваша діяльність на рівні контактів тепер буде видно на часовій шкалі ваших клієнтів.

   :::image type="content" source="media/Contact_Activities2.png" alt-text="Кінцевий результат після настроювання дій контактів":::

### <a name="contact-level-activity-timeline-filtering"></a>Фільтрація часової шкали активності на рівні контактів

Після налаштування зіставлення активності на рівні контактів і її запуску часова шкала активності для ваших клієнтів буде оновлена. Він містить їхні ідентифікатори або імена, залежно від конфігурації *вашого ContactProfile*, для дій, за якими вони діяли. На часовій шкалі можна фільтрувати дії за контактами, щоб побачити конкретні контакти, які вас цікавлять. Крім того, ви можете переглянути всі дії, які не призначені певному контакту, вибравши елемент **Дії, не зіставлені з контактом**.
