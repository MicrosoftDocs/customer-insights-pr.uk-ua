...
Профілі клієнтів або контактів, які відповідають фільтрам визначення сегмента, називаються *членами* сегмента. Застосовуються деякі [ліміти послуг](/dynamics365/customer-insights/service-limits).
...
Сегмент облікових записів або сегмент контактів (попередній перегляд) за допомогою конструктора сегментів: [створення власного](segment-builder.md)

> [!NOTE]
> Більшість напрямків експорту потребують контактної інформації для маркетингових цілей. Тому створіть сегменти контактів, які використовуватимуться для цих експортів.

---

## <a name="manage-existing-segments"></a>Керування наявними сегментами

Перейдіть на сторінку **Сегменти**, щоб переглянути створені сегменти, їх стан і стан, а також час останнього оновлення даних. Список сегментів можна відсортувати за будь-яким стовпцем або скористатися полем пошуку, щоб знайти сегмент, яким потрібно керувати.

> [!TIP]
> У середовищах B-to-B стовпець Тип **аудиторії визначає,** чи сегмент базується на облікових записах або контактах.
...
>
> Список — це попередній перегляд відповідних учасників сегмента. Він показує перші 100 записів вашого сегмента. Це дає змогу швидко оцінити його та за потреби переглянути його визначення. Щоб переглянути всі відповідні записи, виберіть елемент **Додатково**, який відкриває сторінку [**"Сутності"**](entities.md), або [експортує сегмент](export-destinations.md).
