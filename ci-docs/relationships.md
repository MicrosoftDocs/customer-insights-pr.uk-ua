...
- Системні зв'язки, що не підлягають редагуванню, створюються системою в рамках процесу уніфікації даних
- Успадковані зв'язки, які не можна редагувати, створюються автоматично з джерел даних
- Настроювані зв'язки, які можна редагувати, створюються та настроюються користувачами
...
3. В області **Новий зв’язок** введіть зазначені нижче відомості.
   ...
   - **Кардинальність** джерела: Кардинальність вихідної сутності. Кратність описує можливу кількість елементів у наборі. Вона завжди співвідноситься із кратністю цілі. Ви можете вибрати **Один** або **Багато**. Підтримуються лише зв’язки «багато-до-одного» та «один-до-одного».  
   ...
   - **Кардинальність** цілі: кардинальність записів цільової сутності.
   - **Поле** вихідного ключа: поле зовнішнього ключа у вихідній сутності. Приклад: SupportCase використовує **CaseID** як поле зовнішнього ключа.
   - **Поле** цільового ключа: ключове поле цільової сутності. Приклад: клієнт використовує **ідентифікатор** клієнта як ключове поле.
...
Середовища, настроєні на використання корпоративних облікових записів як основної цільової аудиторії, можуть настроювати ієрархії облікових записів для пов'язаних корпоративних облікових записів. Наприклад, окремі організаційні одиниці однієї компанії.

Організації створюють ієрархії бізнес-партнерів, щоб краще керувати бізнес-партнерами та зв'язками між ними. Customer Insights підтримує ієрархії облікових записів батьків і дітей, які вже існують у проковтнутих даних клієнтів. Наприклад, бізнес-партнерів із Dynamics 365 Sales. Ці ієрархії можна настроїти на сторінці "Зв'язки **·**".

1. Відкрийте **Дані** > **Зв'язки**.
1. Виберіть вкладку **Ієрархія бізнес-партнера**.
1. Виберіть **Створити ієрархію бізнес-партнера**.
1. В області **Ієрархія бізнес-партнера** укажіть ім'я для цієї ієрархії. Система створює ім'я для вихідної сутності, але ви можете змінити його.
...
1. **Виберіть UID** облікового запису та **батьківський UID** із вибраної сутності.
1. Натисніть кнопку **Зберегти**, щоб завершити ієрархію облікових записів.

## <a name="manage-existing-relationships"></a>Керування наявними зв’язками

Перейдіть на сторінку **"Зв'язки"**, щоб переглянути всі створені зв'язки, їх вихідну сутність, цільову сутність і кардинальність.
...
**Використовуйте параметри фільтрування за** або **пошук зв'язків**, щоб знайти певний зв'язок. Щоб переглянути мережеву схему існуючих зв'язків та їх кардинальності, виберіть Пункт [**Візуалізатор**](#explore-the-relationship-visualizer).

Виберіть зв'язок, щоб переглянути доступні дії.
...
Щоб настроїти подання, ви можете змінити розташування блоків, перетягуючи їх на полотні. Інші варіанти включають: 
...
Шлях до зв’язку описує сутності, з’єднані за допомогою зв’язків між вихідною сутністю та цільовою сутністю. Він використовується при створенні сегмента або міри, яка включає сутності, відмінні від єдиної профільної сутності, і існує кілька варіантів досягнення єдиної профільної сутності. Різні шляхи до зв'язків можуть приносити різні результати.
