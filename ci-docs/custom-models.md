...
- Ця функція підтримує веб-служби, опубліковані через [конвеєри машинне навчання Azure](/azure/machine-learning/concept-ml-pipelines).
...
1. Ви побачите екран **Робочий цикл збережено** з докладною інформацією про цей робочий цикл.    
   Якщо для конвеєра Azure машинне навчання настроєно робочий цикл, Customer Insights приєднається до робочої області, яка містить конвеєр. Customer Insights отримає роль співавтора **в** робочій області Azure.
...
1. На сторінці Настроювані **моделі** виберіть вертикальні три крапки (&vellip;) у **стовпці Дії** поруч із створеним раніше робочим циклом і натисніть кнопку **Редагувати**.
...
1. Для кожного введеного **веб-сервісу** можна оновити відповідну **сутність** з Customer Insights. Потім виберіть **Далі**.
...
1. На сторінці Настроювані **моделі** виберіть вертикальні три крапки (&vellip;) у **стовпці Дії** поруч із попередньо створеним робочим циклом.

1. Виберіть **Запустити**.

Цей робочий цикл також запускається автоматично з кожним запланованим оновленням. Дізнайтеся докладніше про [настроювання запланованих оновлень](schedule-refresh.md).
