...
**Плитка профілю клієнта**: ця плитка показує різні значення з уніфікованої сутності *Клієнт*. Якщо поле не має значення для вибраного профілю клієнта, воно не відображатиметься, окрім поля адреси. Плитка поділена на розділи:

- У першому розділі відображається попередньо визначений набір полів, а потім — усі поля, які входять до складу покажчика пошуку і фільтрації. Усі поля, пов'язані з адресами, об'єднані в один рядок, який показує, навіть якщо профіль не містить інформації про адресу.
