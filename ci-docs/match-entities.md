...
1. Виберіть порядок **сутностей**. Наприклад, виберіть **eCommerce:eCommerceContacts** як основну сутність, а **LoyaltyScheme:loyCustomers** як другу сутність. 
1. Щоб кожен запис у об'єкті господарювання був унікальним клієнтом і відповідав усій нижченаведеній сутності, виберіть елемент **Включити все**.
1. Виберіть **Готово**. 

Після вказівки порядку відповідності визначені пари збігів відображаються в **розділі Відомості про відповідні записи** на **DataUnifyMatch** > **·** > **·**. Ключові показники порожні, доки процес матчу не завершиться.
...
Основна сутність *eCommerce:eCommerceContacts* зіставляється з сутністю *LoyaltyScheme:loyCustomers*. Набір даних, який є результатом першого кроку відповідності, зіставляється з такою сутністю, якщо у вас більше двох сутностей.
...
1. Виберіть пункт **Додати правило** під сутністю в **розділі Відповідні записи**, щоб визначити правила відповідності.
...
1. У області **Створення правила** налаштуйте умови для правила.
   ...
   - **Точність**: задаєте рівень точності, який застосовуватимуться для цієї умови. 
     - **Базові**: виберіть з-поміж значень *Низький*, *Середній*, *Високий* і *Точний*. Виберіть **"Точний"**, щоб відповідати лише записам, які відповідають 100 відсоткам. Виберіть один із інших рівнів, щоб зіставити записи, які ідентичні не на 100 відсотків.
...
Дедупліковані запис А бізнес-партнера використовуються в процесі перехресного зіставлення сутностей. Дедуплікація відбувається для окремих сутностей і може бути налаштована для кожної сутності, яка використовується в парах матчів.
...
1. У розділі Відомості **про дедупліковані записи** виберіть елемент **Установити сутності**. Якщо правила видалення повторів уже створені, натисніть **Редагувати**.
...
1. В області **Параметри злиття** виберіть сутності, до яких потрібно застосувати видалення повторів.
   ...
   1. За бажанням, щоб визначити правила дедуплікації окремих атрибутів сутності, виберіть " **Додатково"**. Наприклад, ви можете зберегти останню електронну пошту та найповнішу адресу з різних записів. Розгорніть сутність, щоб переглянути всі її атрибути та визначити, який параметр слід використовувати для окремих атрибутів. Якщо ви виберете параметр на основі рекреації, вам також потрібно вказати поле дати/часу, яке визначає рекрецію. 
 
      > [!div class="mx-imgBorder"]
      > ![Правила вилучення дублікатів, крок 1.](media/match-selfconflation.png "Правила вилучення дублікатів, крок 1")

   1. Натисніть кнопку **Готово**, щоб застосувати параметри злиття для дедуплікації.
...
1. Після [запуску процесу](#run-the-match-process) матчу ви побачите статистику дедуплікації в ключових плитках показників.
...
Якщо ви збагатили сутності на рівні джерело даних, виділіть їх перед запуском процесу відповідності. Збагачені сутності можуть покращити ваші результати об'єднання. Для отримання додаткових відомостей [див](data-sources-enrichment.md). 

Збагачена сутність містить вихідні поля джерело даних та збагачені поля. Отже, якщо ви вирішите працювати з збагаченою сутністю, існуюча конфігурація не впливає. Однак, можливо, знадобиться оновити правила відповідності, щоб натомість використовувати збагачені поля.
...
У більшості випадків відповідність сутності призводить до унікальних профілів користувачів із консолідованими даними. Для динамічного вирішення рідкісних випадків помилкових спрацьовувань і помилкових негативів можна визначити винятки для правила матчу. Винятки застосовуються після обробки правил відповідності та уникнення зіставлення всіх записів, які відповідають критеріям винятку.
...
1. Виберіть параметр користувацької відповідності **зі спадного списку Настроюваний тип** і виберіть **Завантажити шаблон**. Вам потрібен окремий шаблон для кожного варіанту матчу.
...
1. Після передавання файли та сутності стануть доступними. Виберіть параметр **Настроювана відповідність** ще раз. Відобразяться параметри для визначення сутностей, які необхідно включити. Виберіть необхідні сутності зі спадного меню та натисніть кнопку **Готово**.
...
1. Застосування настроюваної відповідності залежить від потрібного параметра відповідності. 
   ...
   - Для **спеціального обходу** або **зіставлення** псевдонімів виберіть пункт **Редагувати** в наявному правилі відповідності або створіть нове правило. У розкривному списку Нормалізація виберіть **параметр Користувацький обхід** або **зіставлення** псевдонімів і натисніть кнопку **Готово**.
...
- Самоконфліктність не показує нормалізовані дані в об'єктах дедуплікації. Однак він застосовує нормалізацію внутрішньо під час дедуплікації. Це за задумом для всіх нормалізацій. 
- Якщо параметр семантичного типу видаляється на **етапі карти**, коли правило відповідності використовує зіставлення псевдоніма або настроюваний обхід, нормалізація не застосовуватиметься. Це відбувається тільки в тому випадку, якщо очистити семантичний тип після налаштування нормалізації в правилі відповідності, оскільки семантичний тип буде невідомий.


## <a name="next-step"></a>Наступний крок

Після завершення процесу матчу принаймні для однієї пари матчів перейдіть до кроку [**Злиття**](merge-entities.md).
