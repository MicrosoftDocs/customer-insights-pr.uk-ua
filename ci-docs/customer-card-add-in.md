...
description: Показувати дані профілю клієнта зі статистики клієнтів у програмах Dynamics 365 за допомогою цієї надбудови.
ms.date: 02/02/2022
ms.reviewer: mhart
...
- Щоб ваші дані Dynamics 365 зіставлялися з профілями клієнтів Статистики клієнтів, ми рекомендуємо, щоб [вони потрапили з програми Dynamics 365 за допомогою Microsoft Dataverse роз'єму](connect-power-query.md). Якщо для прийому контактів (або бізнес-партнерів Dynamics 365) використовується інший метод, потрібно переконатися `contactid`, що поле (або `accountid`) встановлено як [первинний ключ для цього джерело даних на кроці карти процесу об'єднання даних](map-entities.md#select-primary-key-and-semantic-type-for-attributes). 
- Щоб переглянути дані, усі користувачі Dynamics 365 надбудови картки клієнта повинні бути [додані як користувачі](permissions.md) в Службі статистики клієнтів.
- [Для пошуку даних для роботи потрібні налаштовані можливості](search-filter-index.md) пошуку та фільтрування в Службі статистики клієнтів.
- Кожен елемент керування надбудовою спирається на певні дані в «Інсайті про клієнтів». Деякі дані та елементи керування доступні лише в середовищах певних типів. Якщо якійсь елемент керування недоступний через вибраний тип середовища, конфігурація надбудови поінформує вас. Дізнайтеся більше про [способи використання середовищ](work-with-business-accounts.md).
  - **Елемент керування «Показник»**: вимагає наявності [налаштованих показників](measures.md) типу «атрибут клієнта».
  - **Контроль** інтелекту: потрібні дані, згенеровані за допомогою [прогнозів або користувацьких моделей](predictions-overview.md).
  - **Елемент керування «Відомості про клієнта»**: усі поля профілю доступні в уніфікованому профілі клієнта.
  - **Елемент керування «Збагачення»**: необхідно застосувати активні [збагачення](enrichment-hub.md) до профілів клієнтів. Надбудова картки підтримує такі збагачення: [бренди](enrichment-microsoft.md), надані корпорацією Майкрософт, інтереси [,](enrichment-microsoft.md) надані корпорацією Майкрософт, і [дані про](enrichment-office.md) залучення Office, надані корпорацією Майкрософт.
...
Залежно від сценарію можна додавати елементи керування до форми **Контактна особа** або **Бізнес-партнер**. Якщо ваше середовище статистики клієнтів призначене для бізнес-облікових записів, ми рекомендували додати елементи керування до форми облікового запису. У цьому разі замініть в описаних нижче кроках «контактна особа» на «бізнес-партнер».
...
1. Перегляньте конфігурацію проковтування даних. Відредагуйте джерело даних для системи Dynamics 365, яка містить GUID ідентифікатора контакту. Якщо GUID ідентифікатора контакту відображається з великими літерами в редакторі Power Query, спробуйте виконати такі дії: 
    ...
    1. Виберіть **нижній регістр**. Перевірте, чи guids у таблиці тепер малі.
    1. Збережіть джерело даних
    1. Запустіть процеси прийому даних, уніфікації та вниз за течією, щоб поширити зміни в GUID. 

Після завершення повного оновлення елементи керування надбудовою Картки клієнта повинні відображати очікувані дані. 
