...
1. Перегляньте та уточніть свої дані на **Power Query сторінці Редагування запитів**. Сутності, які системи визначать в вибраному джерелі даних, відобразяться в області ліворуч.

   :::image type="content" source="media/data-manager-configure-edit-queries.png" alt-text="Діалогове вікно редагування запитів":::

1. Перетворіть свої дані. Виберіть сутність для редагування або перетворення. Використовуйте параметри у вікні Power Query, щоб застосувати трансформування. Кожне перетворення перелічено в розділі **Застосовані кроки**. Power Query надає численні [попередньо побудовані варіанти трансформації](/power-query/power-query-what-is-power-query#transformations).
   ...
   > - Переконайтеся, що тип даних установлено належним чином і він відповідає даним. Наприклад, для полів дати виберіть тип дати.
...
### <a name="data-type-does-not-match-data"></a>Тип даних не збігається з даними

Найпоширеніша невідповідність типу даних виникає, якщо в полі дати не встановлено правильний формат дати.

Дані можуть бути зафіксовані у джерела та повторно проковтнуті. Або виправте трансформацію в рамках Customer Insights. Щоб виправити перетворення:

1. Відкрийте **Дані** > **Джерела даних**.

1. Поруч із джерело даних з пошкодженими даними натисніть кнопку **Редагувати**.

1. Виберіть **Далі**.

1. Виберіть кожен із запитів і знайдіть неправильні перетворення, застосовані в розділі "Застосовані кроки", або стовпці дат, які не було перетворено у форматі дати.

   :::image type="content" source="media/PQ_corruped_date.png" alt-text="Power Query- Редагування з неправильним форматом дати":::

1. Змініть тип даних, щоб він правильно відповідав даним.

1. Виберіть **Зберегти**. Ця джерело даних оновлена.

## <a name="troubleshoot-ppdf-power-query-based-data-source-refresh-issues"></a>Усунення проблем із оновленням джерело даних на основі PPDF Power Query

Якщо дані застаріли або з’являються помилки після джерело даних оновлення, виконайте такі дії:

1. Перейдіть до [Power Platform](https://make.powerapps.com).

1. **Виберіть середовище** для свого екземпляра Customer Insights.

1. Перейдіть до **розділу Потоки** даних.

1. Для потоку даних, який відповідає джерело даних в Customer Insights, виберіть вертикальні три крапки (&vellip;), а потім натисніть кнопку **Відображати журнал** оновлень.

1. Якщо стан **потоку** даних – **успішно**, можливо, змінилося право власності Power Query на джерело даних, що базується на ньому:

   1. Перегляньте розклад оновлення з журналу оновлень.
   1. Встановіть графік нового власника і збережіть налаштування.

1. Якщо стан **потоку** **даних не вдалося виконати**:

   1. Завантажте файл журналу оновлень.
   1. Перегляньте завантажений файл з причини збою.
   1. Якщо помилку не вдалося усунути, виберіть **?** щоб відкрити заявку в службу підтримки. Додайте завантажений файл журналу оновлень.
