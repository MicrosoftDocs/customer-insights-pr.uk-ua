...
## <a name="use-a-contactprofile-semantic-entity-mapping-to-create-contact-level-activities"></a>Створення справ на рівні контактів за допомогою зіставлення семантичної сутності ContactProfile

Після створення *зіставлення семантичної сутності ContactProfile* можна записувати дії контактів. Це дає змогу бачити в часовій шкалі активності для бізнес-партнера, контактна особа якого відповідала за кожну справу. Більшість кроків слідують типовій конфігурації зіставлення активності.

   > [!NOTE]
   > Щоб справи на рівні контактів працювали, потрібно мати атрибути **AccountID** і **ContactID** для кожного запису в даних про дії.

1. [*Визначення зіставлення семантичної сутності ContactProfile*.](#define-a-contactprofile-semantic-entity-mapping) і запустіть семантичне відображення.

1. У розділі «Аналіз аудиторії» виберіть **Дані** > **Дії**.

1. Натисніть кнопку **Додати справу**, щоб створити нову справу.

1. Назвіть справу, виберіть сутність вихідної справи та виберіть первинний ключ сутності справи.

1. **На кроці Зв'язки** створіть непрямий зв'язок між вихідними даними про дії з бізнес-партнерами, використовуючи контактні дані як сутність посередника. Для отримання додаткових відомостей [див](relationships.md#relationship-paths).
   - Приклад зв'язку для справи під назвою *Покупки*:
      - **Придбання Вихідна активність DataКонтактні** > **дані** за атрибутом **ContactID**
      - **Дані** > **облікового запису даних** контактної особи за атрибутом **AccountID**

   :::image type="content" source="media/Contact_Activities1.png" alt-text="Приклад настроювання зв'язку.":::

1. Після настроювання зв'язків натисніть кнопку **Далі** та завершіть конфігурацію зіставлення справ. Докладні кроки зі створення справи [див](activities.md).

1. Запустіть зіставлення активності.

1. Ваші дії на рівні контактів тепер відображатимуться на часовій шкалі клієнта.
...
Після настроювання зіставлення активності на рівні контакту та її запуску часова шкала активності для ваших клієнтів буде оновлено. Вона включає в себе їх ідентифікатори або імена, залежно *від конфігурації ContactProfile*, для дій, на які вони діяли. Ви можете фільтрувати справи за контактами на часовій шкалі, щоб побачити конкретні контакти, які вас цікавлять. Крім того, можна переглянути всі справи, які не призначено певному контакту, вибравши справи **, не зіставлені з контактом**.

   :::image type="content" source="media/Contact_Activities3.png" alt-text="Параметри фільтрації, доступні для справ на рівні контактів.":::
