...
Dynamics 365 Customer Insights забезпечує пряму інтеграцію з Azure Monitor. Журнали ресурсів Azure Monitor дають змогу відстежувати та надсилати журнали до [сховища](https://azure.microsoft.com/services/storage/) Azure, [Azure Log Analytics](/azure/azure-monitor/logs/log-analytics-overview) або передавати їх до [центрів подій](https://azure.microsoft.com/services/event-hubs/) Azure.

Статистика клієнтів надсилає такі журнали подій:

- **Події аудиту**
  - **APIEvent** - дозволяє відстежувати зміни, здійснені за допомогою інтерфейсу Dynamics 365 Customer Insights користувача.
- **Оперативні заходи**
  - **WorkflowEvent** - Робочий процес дозволяє налаштувати [джерела](data-sources.md) даних, об'єднати [і](data-unification.md) збагатити [і,](enrichment-hub.md) нарешті, [експортувати](export-destinations.md) дані в інші системи. Всі ці кроки можуть бути зроблені індивідуально (наприклад, викликати один експорт) або організовані (наприклад, оновлення даних з джерел даних, які запускають процес об'єднання, який буде тягнути додаткові збагачення і після того, як це буде зроблено, експортувати дані в іншу систему). Для отримання додаткових [відомостей див.](#workflow-event-schema)
...
1. У службі customer insights виберіть **systemDiagnostics** > **·**, щоб переглянути призначення діагностики, настроєні в цьому екземплярі.
...
1. **Виберіть клієнта** azure підписки з цільовим ресурсом і виберіть **увійти**.
...
- **Події** аудиту: [події](#api-event-schema) API для відстеження змін конфігурації в службі. `POST|PUT|DELETE|PATCH` операції відносяться до цієї категорії.
- **Операційні події**: [події](#api-event-schema) API або [події](#workflow-event-schema) робочого циклу, створені під час використання служби.  Наприклад, `GET` запити або події виконання робочого циклу.
...
Керівник служби статистики клієнтів отримує **дозвіл постачальника** журналу Analytics на ресурсі. Журнали будуть доступні в розділі **LogsTablesLog** > **·** > **управління** у вибраній робочій області Журнал Analytics. **Розгорніть рішення керування журналами** та знайдіть `CIEventsAudit` таблиці та `CIEventsOperational` таблиці.
...
Події API та події робочого циклу мають спільну структуру та деталі, де вони відрізняються, див [...](#api-event-schema)[...](#workflow-event-schema).
...
| `category`        | String    | Обов'язковий          | Журнал категорії події. Або `Operational` .`Audit` Всі ПОСТ / PUT / PATCH / ВИДАЛИТИ HTTP Запити позначені `Audit`, все інше з`Operational` | `2020-09-08T09:48:14.8050869Z`                                                                                                                                           |
...
| `identity`        | String    | Необов'язково          | Об'єкт JSON, що описує особу користувача або програми, яка зробила операцію.       | Дивіться [розділ Посвідчення](#identity-schema).     |  |
...
| `Claims`                      | Претензії користувача або веб-маркера програми JSON (JWT). Властивості претензії залежать від організації та конфігурації Azure Active Directory. |
...
Робочий цикл містить кілька кроків. [Використання джерел](data-sources.md) даних, [об'єднання](data-unification.md), [збагачення](enrichment-hub.md) та [експорт](export-destinations.md) даних. Всі ці кроки можуть виконуватися індивідуально або організовано за допомогою наступних процесів. 
...
| `properties.operationType`                   | Так      | Так  | Ідентифікатор операції див. (#operation типів)                                                                                                                                                                                       |
...
| `properties.identifier`                      | No       | Так  | - Для OperationType = `Export`, ідентифікатор є guid конфігурації експорту. <br> - Для OperationType = `Enrichment`, це керівництво збагачення <br> - Для OperationType `Measures` і `Segmentation`, ідентифікатор є ім'ям сутності. |
...
| `properties.additionalInfo.Kind`             | No       | Так  | Необов'язково. Лише для операції Тип `Export`. Визначає тип експорту. Для отримання додаткових відомостей [див](export-destinations.md).                                                                                          |
| `properties.additionalInfo.AffectedEntities` | No       | Так  | Необов'язково. Лише для операції Тип `Export`. Містить список настроєних сутностей під час експорту.                                                                                                                                                            |
| `properties.additionalInfo.MessageCode`      | No       | Так  | Необов'язково. Лише для операції Тип `Export`. Детальне повідомлення для експорту.                                                                                                                                                                                 |
| `properties.additionalInfo.entityCount`      | No       | Так  | Необов'язково. Лише для операції Тип `Segmentation`. Указує загальну кількість членів сегмента.                                                                                                                                                    |
